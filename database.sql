...
INSERT INTO `build_guia` VALUES (1,1,1,NULL,1,'Recarga de Energia%','HP%','Bônus de cura%','Energia, HP','220% de recarga de energia'),(2,2,2,NULL,2,'ATK% ou Recarga de Energia%','Bônus Pyro%','CRIT Rate&','Recarga de Energia, CRIT Rate, CRIT DMG, ATK%','220-235% de Recarga de Energia'),(3,3,3,'É a arma assinatura da Furina e balancea buffs com o aumento e diminuição da vida, que acontece na utilização da Habilidade Elemental da Furina.',3,'Recarga de Energia% ou HP%','HP%','CRIT Rate%','Recarga de Energia%, HP%, CRIT Rate, CRIT DMG','200% de Recarga de Energia; 40k de HP'),(8,9,16,NULL,6,'Recarga de Energia%','Bônus Dendro%','Taxa Crítica%','Recarga de Energia, Proeficiência Elemental, Taxa Crítica.','Recarga de Energia +180%.'),(9,8,8,NULL,7,'Proeficiência Elemental','Proeficiência Elemental','Proeficiência Elemental ou Crit Rate','Proeficiência Elemental, Crit Rate, Crit DMG','Proeficiência Elemental 1000; Recarga de Energia 160 - 200%.'),(10,11,6,'É sua arma assinatura, aumentado seu dano.',10,'HP%','Bônus Hydro%','Taxa Crítica%','Recarga de Energia, HP, Crit Rate','Recarga de Energia +170%'),(11,10,12,'Sua arma assinatuua, aumentando seu dano e recarga de energia.',10,'Recarga de Energia%','Bônus Electro%','Taxa Crítica%','ATK, Crit DMG, Recarga de Energia','Recarga de Energia +200%'),(12,4,2,'Bom para o dano daas habilidades do Kaeya.',1,'ATK%','Bônus Cryo%','Crit DMG%','ATK, Crit Rate, Crit DMG','REcarga de Energia 200%'),(13,5,6,'Paasiva universal e alto valor de DMG bônus',3,'ATK%','Bõnus Electro%','Taxa Crítica%','Crit Rate,  Crit DMG, ATK','ATK 2000; Crit Rate +60%.'),(14,6,8,NULL,9,'Proeficiência Elemantal','Proeficiência Elemental','Proeficiência Elemental','Recarga de Energia, Proeficiência Elemental','Recarga de Energia +170%'),(15,7,10,'Aumenta tanto a defesa quanto o Crti DMG.',5,'DEF%','Bônus Geo%','Taxa Crítica%','DEF%, Crit DMG, Crit Rate ','Recarga de Energia +180%');
...
INSERT INTO `build_usuario` VALUES (1,19,3,4,3,'Furina Support',0,'HP%','HP%','Dano Crítico','Escolhi a favonius pois a recarga de energia vai fazer ser mais fácil de ter energia para a burst da furina, e conceitrei os artefatos em vida e dano crítico para aumentar o dano da habilidade elemental dela, melhorando suas capacidades de suporte.','2025-11-08 21:37:19'),(5,19,1,4,1,'Bennett Healer',0,'HP%','HP%','Bônus de Cura','Escolhi a favonius por ser uma arma ótima para recarga de energia, o que é muito importante para a burst do Bennett ficar sempre carregada, já que é ela que vai possibilitar ele curar o time.','2025-11-24 22:52:29'),(6,20,7,11,5,'Noelle Shield',0,'DEF%','DEF%','DEF%','Maximizar o pontencial de shield da Noelle com a concentração na defesa, que influencia também na cura fornecida e no dano dado. Assim tornando ela uma ótima sustain e healer para o time.','2025-11-25 02:21:30'),(7,20,1,4,1,'Bennett Support',1,'Recarga de Energia%','ATK%','Taxa Crítica','Aumentar a recarga de energia do Bennett, e concentrar os substatus em dano crítico e atk para melhorar seu desempenho como um Sub-DPS.','2025-11-25 02:24:08'),(8,21,10,12,10,'Raiden DPS',0,'ATK%','Bônus Electro','Taxa Crítica','Concentrar os substatus em recarga de energia e ATK, para aumentar o dano dela on-field e concentrar na evolução do talento da burst, que vai ser de onde o dano dela vai vir.','2025-11-25 02:27:35'),(9,21,11,15,10,'Yelan Sub-DPS',0,'HP%','Bônus Hydro','Dano Crítico','Graças a capacidade de reset de cooldown da arma que eu coloquei, vai ser muito mais fácil de se ter a todo momento a burst da Yelan, que funciona sem ela no campo, então vai ser um ótimo suporte para o DPS on-field. Tendo seu dano aumentado usando substatus de HP.','2025-11-25 02:30:36'),(10,23,7,10,5,'Noelle DPS',0,'ATK%','Bônus Geo','Dano Crítico','Concentrei os status em dar dano, e coloquei uma arma que aumenta tanto o ATK quanto a DEF, que escala com o dano e o escudo dela.','2025-11-25 02:35:13'),(11,22,2,5,4,'Viajante DPS',0,'ATK%','Bônus Pyro','Taxa Crítica','Concentrei os substatus em taxa e dano crítico, para aumentar o dano em campo dela, concentrando a evolução de talentos na Habilidade Elemental que aumenta o dano elemental da viajante em campo. E a arma aumenta o dano elemental também, sendo uma boa escolha.','2025-11-25 02:38:43'),(12,22,5,7,3,'Fischl Sub-DPS',0,'Proficiência Elemental','Bônus Electro','Proficiência Elemental','Escolhi uma arma que ajuda no dano da habilidade elemental da Fischl, que será o ponto principal dessa build, onde será concentrado seu dano off-field, que será de ajuda ao DPS em campo. Concentrando-me também na proficiência elemental para aumentar seu dano elemental e das reações.','2025-11-25 21:28:20');
...
CREATE TABLE `favorito` (
  ...
  `id_usuario` int NOT NULL,
  `id_build_guia` int DEFAULT NULL,
  `id_build_user` int DEFAULT NULL,
  `data_adicionado` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_favorito`),
  UNIQUE KEY `unique_fav_guia` (`id_usuario`,`id_build_guia`),
  UNIQUE KEY `unique_fav_user` (`id_usuario`,`id_build_user`),
  KEY `id_build_guia` (`id_build_guia`),
  KEY `id_build_user` (`id_build_user`),
  CONSTRAINT `favorito_ibfk_1` FOREIGN KEY (`id_usuario`) REFERENCES `usuarios` (`idUser`),
  CONSTRAINT `favorito_ibfk_2` FOREIGN KEY (`id_build_guia`) REFERENCES `build_guia` (`id_build_guia`) ON DELETE CASCADE,
  CONSTRAINT `favorito_ibfk_3` FOREIGN KEY (`id_build_user`) REFERENCES `build_usuario` (`id_build_user`) ON DELETE CASCADE,
  CONSTRAINT `check_tipo_favorito` CHECK ((((`id_build_guia` is not null) and (`id_build_user` is null)) or ((`id_build_guia` is null) and (`id_build_user` is not null))))
...
INSERT INTO `favorito` VALUES (10,19,1,NULL,'2025-11-25 02:04:03'),(11,20,NULL,5,'2025-11-25 02:19:05'),(12,21,2,NULL,'2025-11-25 02:30:52'),(13,21,3,NULL,'2025-11-25 02:30:59'),(14,21,NULL,5,'2025-11-25 02:31:09'),(15,23,NULL,9,'2025-11-25 02:35:37'),(16,23,NULL,1,'2025-11-25 02:35:47'),(17,22,NULL,8,'2025-11-25 02:38:57'),(18,22,NULL,6,'2025-11-25 02:39:11');
...
INSERT INTO `usuarios` VALUES (1,'lele','lele@gmail.com','senha',NULL,1),(3,'giselle','kupac@ufrrj.br','1234',NULL,0),(19,'leticia','le23@gmail.com','123',NULL,0),(20,'Rain','rain@gmail.com','5678',NULL,0),(21,'Moon','moon@gmail.com','2345',NULL,0),(22,'Lumine','lumine@gmail.com','6543',NULL,0),(23,'Sun','sun@gmail.com','3456',NULL,0);
...
-- Dump completed on 2025-11-25 18:43:02
